...
- (LQNoCopyTextField *)currentDigitalTextField
{
    if (_currentDigitalTextField == nil)
    {
        ...
        UILabel *unitLable = [self setUnitLableWithText:@"吨"];
        [unitLable mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(nameLable.mas_top);
            make.right.equalTo(self.backgroundView.mas_right).with.offset(-15);
...
- (LQNoCopyTextField *)remainingAmountTextField
{
    ...
    {
        UILabel *nameLable = [self setNameLableWithText:@"剩余水量："];
        [nameLable mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"SY00038(充值)";
...
- (void)doLoading
{
    self.currentDigitalTextField.text = @"728.73";
    self.remainingAmountTextField.text = @"723.23";
